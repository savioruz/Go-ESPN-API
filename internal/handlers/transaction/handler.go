// Package transaction wires the transaction HTTP endpoints.
package transaction
...
import (
	...
	"go-espn-api/internal/domains/transaction/service"
...
// Handler serves the transaction endpoints.
type Handler struct {
...
func New(service service.Transaction, otl otel.Otel) Handler {
	return Handler{service: service, otel: otl}
}

// Router registers transaction routes.
...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	ctx, scope := h.otel.NewScope(r.Context(), constant.OtelHandlerScopeName, constant.OtelHandlerScopeName+".transaction.List")
	defer scope.End()
...
func (h *Handler) GetByID(w http.ResponseWriter, r *http.Request) {
	ctx, scope := h.otel.NewScope(r.Context(), constant.OtelHandlerScopeName, constant.OtelHandlerScopeName+".transaction.GetByID")
	defer scope.End()
