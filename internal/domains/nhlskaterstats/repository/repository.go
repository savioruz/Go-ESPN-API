// Package repository provides data access for the NHL skater-stats domain.
package repository
...
// ListFilter holds the NHL skater-stats list query parameters.
type ListFilter struct {
	Season   string
	Ordering string
}

// NHLSkaterStats defines read access for NHL skater season stats.
type NHLSkaterStats interface {
	List(ctx context.Context, f ListFilter, page, pageSize int) ([]dto.SkaterStatsRow, error)
	Count(ctx context.Context, f ListFilter) (int, error)
	GetByID(ctx context.Context, id int64) (*dto.SkaterStatsRow, error)
...
// New creates a new NHL skater-stats repository.
func New(db *postgres.Connection, otl otel.Otel) NHLSkaterStats {
	return &repositoryImpl{db: db, otel: otl}
}

const skaterSelect = "SELECT " + dto.SkaterStatsColumns + " FROM nhl_skater_season_stats"

var skaterOrdering = map[string]string{
	"points":     "points",
	"goals":      "goals",
	"assists":    "assists",
...
func skaterConditions(f ListFilter, args map[string]any) string {
	conds := []string{}
...
func (repo *repositoryImpl) List(ctx context.Context, f ListFilter, page, pageSize int) ([]dto.SkaterStatsRow, error) {
	ctx, scope := repo.otel.NewScope(ctx, constant.OtelRepositoryScopeName, constant.OtelRepositoryScopeName+".nhl_skater_stats.List")
	defer scope.End()

	args := map[string]any{"limit": pageSize, "offset": (page - 1) * pageSize}
	order := drf.ResolveOrdering(f.Ordering, skaterOrdering, "season DESC, points DESC")
	query := skaterSelect + skaterConditions(f, args) + " ORDER BY " + order + ", id LIMIT :limit OFFSET :offset"
	scope.SetAttribute(constant.OtelQueryAttributeKey, query)

	items := []dto.SkaterStatsRow{}
...
func (repo *repositoryImpl) Count(ctx context.Context, f ListFilter) (int, error) {
	ctx, scope := repo.otel.NewScope(ctx, constant.OtelRepositoryScopeName, constant.OtelRepositoryScopeName+".nhl_skater_stats.Count")
	defer scope.End()

	args := map[string]any{}
	query := "SELECT COUNT(id) FROM nhl_skater_season_stats" + skaterConditions(f, args)
	scope.SetAttribute(constant.OtelQueryAttributeKey, query)
...
func (repo *repositoryImpl) GetByID(ctx context.Context, id int64) (*dto.SkaterStatsRow, error) {
	ctx, scope := repo.otel.NewScope(ctx, constant.OtelRepositoryScopeName, constant.OtelRepositoryScopeName+".nhl_skater_stats.GetByID")
	defer scope.End()

	query := skaterSelect + " WHERE id = :id"
	scope.SetAttribute(constant.OtelQueryAttributeKey, query)

	var row dto.SkaterStatsRow
...
func (repo *repositoryImpl) PlayersByIDs(ctx context.Context, ids []int64) (map[int64]playerdto.PlayerResponse, error) {
	ctx, scope := repo.otel.NewScope(ctx, constant.OtelRepositoryScopeName, constant.OtelRepositoryScopeName+".nhl_skater_stats.PlayersByIDs")
	defer scope.End()
	...
	if len(teamIDs) > 0 {
		...
		scope.SetAttribute(constant.OtelQueryAttributeKey, tQuery)
